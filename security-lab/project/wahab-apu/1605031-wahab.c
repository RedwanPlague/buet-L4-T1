...
int setHardwareAddress(int sock, char *myInterface) {
    ...
    sscanf(randAddr, "%x:%x:%x:%x:%x:%x", randomMACAddress + 0, randomMACAddress + 1, randomMACAddress + 2,
           randomMACAddress + 3, randomMACAddress + 4, randomMACAddress + 5);
    for (i = 0; i < 6; ++i)
...
int makeDHCPDiscoverPacket(int sock) {
    ...
    if (DEBUG) {
        printf("DHCPDISCOVER to %s port %d\n", inet_ntoa(socketAddressBroadcast.sin_addr),
               ntohs(socketAddressBroadcast.sin_port));
    ...
    if (DEBUG)
        printf("DHCPDISCOVER to %s port %d\nDHCPDISCOVER XID: %lu (0x%X)\nDHCPDISCOVER ciaddr:  %s\nDHCPDISCOVER "
               ...
               (unsigned long)ntohl(discoverPacket.xid), ntohl(discoverPacket.xid), inet_ntoa(discoverPacket.ciaddr),
               inet_ntoa(discoverPacket.yiaddr), inet_ntoa(discoverPacket.siaddr), inet_ntoa(discoverPacket.giaddr));
...
int makeDHCPRequestPacket(int sock, struct in_addr server_ip) {
    ...
    if (DEBUG)
        printf("DHCPREQUEST to %s port %d\nDHCPREQUEST XID: %lu (0x%X)\nDHCPREQUEST ciaddr:  %s\nDHCPREQUEST yiaddr:  "
               ...
               inet_ntoa(socketAddressBroadcast.sin_addr), ntohs(socketAddressBroadcast.sin_port),
               (unsigned long)ntohl(requestPacket.xid), ntohl(requestPacket.xid), inet_ntoa(requestPacket.ciaddr),
               inet_ntoa(requestPacket.yiaddr), inet_ntoa(requestPacket.siaddr), inet_ntoa(requestPacket.giaddr));
...
long    tv_usec;        // microseconds
    ...
    if (!FD_ISSET(sock, &readfds)) {
        ...
        receivedData =
            recvfrom(sock, (char *)buffer, buffer_size, MSG_PEEK, (struct sockaddr *)&sourceAddress, &address_size);
        if (DEBUG)
    ...
    while (1) {
        ...
        if (DEBUG) {
            ...
            printf("DHCPOFFER ciaddr: %s\nDHCPOFFER yiaddr: %s\nDHCPOFFER siaddr: %s\nDHCPOFFER giaddr: %s\n\n",
                   inet_ntoa(offer_packet.ciaddr), inet_ntoa(offer_packet.yiaddr), inet_ntoa(offer_packet.siaddr),
